...
=item <!--#include file="/path/to/file" -->
...
FIXME If the name of a node doesn't resolve in the template,
it is outputed verbatim. 
...
See L<TAG ATTRIBUTES/action> below. FIXME
...
Will match any regular tag (that is, not comments nor text) that 
isn't explicitly matched. See FIXME
...
=item pre, intro, prechildren, prechild, postchild, postchildren, extro, post
...
If interpolation is enabled (see FIXME), XPath expressions delimited 
...
The expressions' delimiter can be modified to be something else
via FIXME.
...
Example: 
    ...
    # will transform <foo>blah</foo> into <foo>replacement text</foo>

    $template->( 'text()' => { action => DO_TEXT_AS_CHILD,
                               pre => 'text: '   } );
    # will transform <foo>blah</foo> into <foo>text: blah</foo>
...
the tag. When invoked, the subroutine is passed two parameters: the 
current node's object and a tag object holding all the attributes 
of the visited tag. Modifications to the tag object only affect 
